<?php
					  ...
					  $op2op )
	{
		...
		// update the vupdated to ensure it doesn't timeout in routing
		if ( ( $status == 2 ) && $op2op )
			$query = "UPDATE p_requests SET status = 1, vupdated = $now, tupdated = 1, tloop = 0 AND op2op = 0 WHERE requestID = $requestid" ;
		else
			$query = "UPDATE p_requests SET status = 1, vupdated = $now, tupdated = 1, tloop = 0 WHERE requestID = $requestid AND ( opID = $opid OR op2op = $opid OR opID = 1111111111 ) AND status = 0" ;
		database_mysql_query( $dbh, $query ) ;
