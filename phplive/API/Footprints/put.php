<?php
					...
					$longitude = 0 )
	{
		...
		{
			// only insert new visitor on page load, not subsequent footprint calls with no title
			$query = "INSERT INTO p_footprints_u VALUES ( NULL, '$vis_token', $now, $now, $deptid, $marketid, 0, $os, $browser, $footprints, $requests, $initiates, '$resolution', '$ip', '$onpage', '$title', '$refer', '$country', '$region', '$city', $latitude, $longitude )" ;
			database_mysql_query( $dbh, $query ) ;
			if ( $dbh[ 'ok' ] ) { return 1 ; }
		}
		return 0 ;
					...
					$refer )
	{
		...
		if ( !$refer && $marketid && isset( $marketinfo ) && isset( $marketinfo["marketID"] ) )
			$refer = $marketinfo["name"] ;
		else if ( !$refer && ( !is_numeric( $marketid ) || !$marketid ) )
			return false ;
