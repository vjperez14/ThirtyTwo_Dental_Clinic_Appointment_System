<?php
	...
	FUNCTION Ops_update_itr_IdleOps( &$dbh )
	{
		...
		$query = "SELECT opID, mapp FROM p_operators WHERE ( ( lastactive < $idle AND mapp = 0 ) OR ( lastactive < $idle_mapp AND mapp = 1 ) ) AND status = 1 AND lastactive <> 0" ;
		...
		if ( $dbh[ 'ok' ] )
		{
			if ( database_mysql_nresults( $dbh ) )
			{
				...
				$query = "UPDATE p_operators SET status = 0 WHERE ( ( lastactive < $idle AND mapp = 0 ) OR ( lastactive < $idle_mapp AND mapp = 1 ) ) AND status = 1 AND lastactive <> 0" ;
				database_mysql_query( $dbh, $query ) ;

				$ops_offline = "" ; // for logging purposes
