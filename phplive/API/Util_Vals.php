<?php
	...
	FUNCTION Util_Vals_WriteToConfFile( $valname, $val )
	{
		...
		if ( is_file( "$CONF[CONF_ROOT]/config.php" ) )
		{
			file_put_contents( "$CONF[CONF_ROOT]/config.php", $conf_string, LOCK_EX ) ;
			if ( $opcache ) { opcache_invalidate( "$CONF[CONF_ROOT]/config.php" ) ; }
			return true ;
		}
		else { return false ; }
	}
	...
	FUNCTION Util_Vals_WriteToFile( $valname, $val )
	{
		...
		if ( is_file( "$CONF[CONF_ROOT]/vals.php" ) )
		{
			file_put_contents( "$CONF[CONF_ROOT]/vals.php", $conf_string, LOCK_EX ) ;
			if ( $opcache ) { opcache_invalidate( "$CONF[CONF_ROOT]/vals.php" ) ; }
			return true ;
		}
		else { return false ; }
	}
	...
	FUNCTION Util_Vals_WriteVersion( $version )
	{
		...
		file_put_contents( "$CONF[CONF_ROOT]/VERSION.php", $version_string, LOCK_EX ) ;
		if ( $opcache ) { opcache_invalidate( "$CONF[CONF_ROOT]/VERSION.php" ) ; }
	}
?>